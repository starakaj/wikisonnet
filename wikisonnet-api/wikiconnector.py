...
def writePoem(host, page_id, poem_id):
    ## Write the poem
    ...
    ## Store the poem
    conn = mysql.connector.connect(user='william', password='Sh4kespeare', host=host, database='wikisonnet')
    cursor = conn.cursor()
    query = (
...
def writePoemAsync(host, page_id, poem_id):
    p = Process(target=writePoem, args=(host, page_id, poem_id))
    p.start()

def getRandomPoemTitle(host='localhost'):
    conn = mysql.connector.connect(user='william', password='Sh4kespeare', host=host, database='wikisonnet')
    cursor = conn.cursor()
...
def getCachedPoemForPage(host='localhost', page_id=21, complete=True):
    conn = mysql.connector.connect(user='william', password='Sh4kespeare', host=host, database='wikisonnet')
...
def getSpecificPoem(host='localhost', poem_id=181):
    conn = mysql.connector.connect(user='william', password='Sh4kespeare', host=host, database='wikisonnet')
    cursor = conn.cursor(dictionary=True)
...
def writeNewPoemForPage(host='localhost', page_id=21):
    d = {}
    d['complete'] = 0
    d['starting_page'] = page_id
    d['id'] = 1337
    writePoemAsync(host, page_id)
    return d;

def writeNewPoemForPage(host='localhost', pageID=21):
    ## Create the row for the cached posStringForPoemLines
    write_conn = mysql.connector.connect(user="william", password="Sh4kespeare", host=host, database="wikisonnet")
    ...
    writePoemAsync(host, pageID, poem_id)
